package com.skriptide.core.skript
...
class SkriptParser{
    ...
    private fun parse(index: Int, parent: Node, split: List<String>, added: Vector<Int>): Int {
        ...
        for (x in index + 1 until split.size) {
            ...
            if (current == "" || added.contains(x)) continue
            if (currentTabCount == (parent.tabLevel + 1)) {
                before = Node(parent, current.trim().replace("\r","").replace("\t",""), currentTabCount, x + 1)
                parent.childNodes.add(before)
                added.add(x)
                continue
            }
            if (currentTabCount > (parent.tabLevel + 1)) {
                val node = Node(parent, current.trim().replace("\r","").replace("\t",""), currentTabCount, x+ 1)
                toSkip += parse(x, node, split, added) - 1
