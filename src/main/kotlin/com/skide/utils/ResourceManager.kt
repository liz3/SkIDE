...
class ResourceManager(val coreManager: CoreManager){
    ...
    fun loadResources(callback: (Int, Int, String) -> Unit){
        ...
        if (!skriptVersionsFolder.exists()){
            skriptVersionsFolder.mkdir()
        }
            ...
            if (addonName != "Skript" && addonName != "skript"){
                ...
                val author = {
                    ...
                    addonDevs.forEach{ dev ->
                        ...
                        if (dev.getString("addon") == addonName.split(" ").first()){
                            devName = dev.getString("author")
                        }
                    }
                ...
                if (author == ""){
                    println("dev null for $id $addonName in allSyntaxes")
                }
                ...
                if (!addons.containsKey(addonName)){
                    addons[addonName] = Addon((addons.size + 1).toLong(), addonName, author)
                }

                val addon = addons[addonName]!!

                if (addonVersion == null || addonVersion == ""){
                    addonVersion = "0.0.0"
                }

                if (!addon.versions.containsKey(addonVersion)){
                    addon.versions[addonVersion] = Vector()
                }
