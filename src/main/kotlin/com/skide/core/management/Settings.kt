...
class ConfigManager(val coreManager: CoreManager){
    ...
    var skUnityKey = ""
        set(value){
            ...
            if (configLoaded){
                writeFile(getConfigObject().toString().toByteArray(), configFile)
            }
        }
    ...
    fun addProject(holder: PointerHolder): Boolean{
        if (projects.containsKey(holder.id)){
            return false
        }

        projects[holder.id] = holder
    ...
    fun addServer(holder: PointerHolder): Boolean{
        if (servers.containsKey(holder.id)){
            return false
        }
    ...
    private fun readConfig(): Boolean{
        ...
        if (readResult.first == FileReturnResult.SUCCESS){
            ...
            if (obj.has("skunity_key")){
                skUnityKey = obj.getString("skunity_key")
            }

            if (obj.has("skunity_name")){
                skUnityUsername = obj.getString("skunity_name")
            }
        ...
        //change configurations so theme is variable
        if(get("theme") == "dark"){
            set("theme", "Dark")
        }
    ...
    private fun createFiles(): Boolean{
        ...
        if (!defaultServerPath.exists()){
            defaultServerPath.mkdir()
        }

        if (writeFile(getConfigObject().toString().toByteArray(), configFile, false, true).first != FileReturnResult.SUCCESS){
            return false
        }

        if (writeFile(brackets, projectsFile, false, true).first != FileReturnResult.SUCCESS){
            return false
        }

        if (writeFile(objForServer.toString().toByteArray(), serversFile, false, true).first != FileReturnResult.SUCCESS){
            return false
        }

        if (writeFile(brackets, hostsFile, false, true).first != FileReturnResult.SUCCESS){
            return false
        }
    ...
    private fun getConfigObject(): JSONObject{
        ...
        if (skUnityKey != ""){
            obj.put("skunity_key", skUnityKey)
        }

        if (skUnityUsername != ""){
            obj.put("skunity_name", skUnityUsername)
        }
