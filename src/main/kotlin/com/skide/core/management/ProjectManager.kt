...
class ProjectManager(val coreManager: CoreManager){
    ...
    private fun loadProjectConfiguration(holder: PointerHolder): Pair<ProjectConfigurationLoadResult, Project?>{
        ...
        if (readResult.first == FileReturnResult.NOT_FOUND){
            return Pair(ProjectConfigurationLoadResult.NOT_FOUND, null)
        }
...
class ProjectFileManager(val project: Project){
    ...
    fun delCompileOption(name: String){
        if (!compileOptions.containsKey(name)){
            return
        }
    ...
    fun addCompileOption(opt: CompileOption){
        if (compileOptions.containsKey(opt.name)){
            return
        }
    ...
    fun writeCompileOptions(){
        ...
        compileOptions.values.forEach{ c ->
            ...
            c.excludedFiles.forEach{
                excluded.put(it.absolutePath)
            }

            c.includedFiles.forEach{
                included.put(it.absolutePath)
            }
    ...
    fun deleteFile(rName: String): Boolean{
        if (!projectFiles.containsKey(rName)){
            return false
        }
        ...
        compileOptions.values.forEach{
            if (it.includedFiles.contains(file)){
                it.includedFiles.remove(file)
            }

            if (it.excludedFiles.contains(file)){
                it.excludedFiles.remove(file)
            }
        }
    ...
    fun reNameFile(rName: String, newNameRaw: String): Boolean{
        val newName = if (newNameRaw.contains(".")){
            newNameRaw
        }else{
            "$newNameRaw.sk"
        }
