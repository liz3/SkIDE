...
object Menus{
    ...
    fun getMenuForArea(codeManager: CodeManager, x: Double, y: Double): ContextMenu{
        ...
        for ((name, opt) in codeManager.findHandler.project.openProject.project.fileManager.compileOptions){
            ...
            compileEntry.setOnAction{
                codeManager.findHandler.project.openProject.guiHandler.openFiles.forEach{
                    it.value.saveCode()
                }

                codeManager.findHandler.project.openProject.compiler.compile(codeManager.findHandler.project.openProject.project, opt, codeManager.findHandler.project.openProject.guiHandler.lowerTabPaneEventManager.setupBuildLogTabForInput())
            }

            compileMenu.items.add(compileEntry)
        }

        if (codeManager.area.selectedText.isNotEmpty()){
            menu.items.add(copyEntry)
        }

        menu.items.add(pasteEntry)

        if (codeManager.findHandler.project.coreManager.skUnity.loggedIn){
            menu.items.add(skUnityEntry)
        }
    ...
    fun getMenuForProjectFile(project: OpenProject, holder: File, parent: TreeView<String>, x: Double, y: Double): ContextMenu{
        ...
        deleteItem.setOnAction{
            if (Prompts.infoCheck("Delete file", "Delete ${holder.name}", "Are you sure you want to delete this file?", Alert.AlertType.CONFIRMATION)) {
