...
class OpenProjectGuiManager(val openProject: OpenProject, val coreManager: CoreManager){
    ...
    fun startGui(): ProjectGuiEventListeners{
        //needs to be set into it's own variable configuration
    ...
    fun switchMode(mode: EditorMode){
        ...
        if (mode == EditorMode.SIDE_SPLIT){
            ...
            if (basePane.tabs.size != 0){
                basePane.selectionModel.select(0)
            }
        }
...
class ProjectGuiEventListeners(private val openProjectGuiManager: OpenProjectGuiManager, val controller: ProjectGUIController, val coreManager: CoreManager){
    ...
    private fun setupNewTabForDisplay(holder: OpenFileHolder){
        Platform.runLater{
            ...
            if (holder.name.endsWith(".sk")){
                holder.codeManager.setup(holder)
            }else{
                ExternalHandler(holder)
            }
    ...
    private fun updateStructureTab(holder: OpenFileHolder){
        if (structureTab.first.isDisabled){
            structureTab.first.isDisable = false
        }
        ...
        filesTab.second.setOnMouseReleased{ev ->
            if (contextMenuVisible == null){
                if (ev.button == MouseButton.SECONDARY){
                    if (filesTab.second.selectionModel.selectedItem == null){
                        return@setOnMouseReleased
                    }

                    val selectedItem = filesTab.second.selectionModel.selectedItem as TreeItem<String>

                    contextMenuVisible = if (selectedItem == filesTab.second.root){
                        Menus.getMenuForRootProject(openProjectGuiManager.openProject, filesTab.second, ev.screenX, ev.screenY)
                    }
                    else {
    ...
    private fun setupMainMenu(){
        ...
        skUnity.setOnAction{
            if (coreManager.skUnity.login()){
                controller.mainBenuBar.menus[2].items.remove(skUnity)
            }
        }
        ...
        fileMenu.setOnShowing{
            ...
            coreManager.configManager.projects.values.forEach{
                val open = coreManager.projectManager.openProjects.any{
                    openProject -> it.name == openProject.project.name
                }
    ...
    private fun registerEditorEvents(){
        ...
        controller.toggleTreeViewButton.setOnAction{
            ...
            if (browserVisible){
                ...
                pane.maxWidth = 1500.0 //needs adjusted based off of window size
